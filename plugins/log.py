...
def record_log_messages ( m ):
    if m.content_type == 'text':
        ...
        if m.chat.id > 0:  # private chat
            message += str( m.from_user.username ) + " (" + str( m.chat.id ) + "): " + m.text
        else:  # group chat
            message += str( m.from_user.username ) + '(' + str( m.from_user.id ) + ') in "' + m.chat.title + '"[' + str(
                m.chat.id ) + ']: ' + m.text
...
def record_log_queries ( q ):
    query = time.strftime( "%d/%m/%y %H:%M:%S" ) + " | " + str( q.from_user.username ) + " (" + str(
